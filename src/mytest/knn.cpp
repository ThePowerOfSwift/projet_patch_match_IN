...
void k_improve_guess(cv::Mat * a, cv::Mat * b, /*int ax, int ay, */ int &xbest,
		     int &ybest, int &kdbest, int bx, int by, bool & guess_ok)
{
...
void trier_voisins_init(cv::Mat * knn, cv::Mat * knnd, int x, int y)
{
	...
	if ((knnd->at < cv::Vec < float,
	     k > >(y, x))[0] > (knnd->at < cv::Vec < float, k > >(y, x))[1]) {
		float inter = (knnd->at < cv::Vec < float, k > >(y, x))[0];

		(knnd->at < cv::Vec < float, k > >(y, x))[0] =
		    (knnd->at < cv::Vec < float, k > >(y, x))[1];

		(knnd->at < cv::Vec < float, k > >(y, x))[1] = inter;

		inter = (knn->at < cv::Vec < float, k > >(y, x))[0];

		(knn->at < cv::Vec < float, k > >(y, x))[0] =
		    (knn->at < cv::Vec < float, k > >(y, x))[1];
...
void trier_voisins(cv::Mat * knn, cv::Mat * knnd, int x, int y)
{
	...
	for (int i = k - 2; i > -1; i--) {
		if (valeur < (knnd->at < cv::Vec < float, k > >(y, x))[i]) {
			(knnd->at < cv::Vec < float, k > >(y, x))[i + 1] =
			    (knnd->at < cv::Vec < float, k > >(y, x))[i];
			(knnd->at < cv::Vec < float, k > >(y, x))[i] = valeur;

			(knn->at < cv::Vec < float, k > >(y, x))[i + 1] =
			    (knn->at < cv::Vec < float, k > >(y, x))[i];
