...
void improve_guess(cv::Mat *a, cv::Mat *b, int &xbest, int &ybest, int &dbest, int bx, int by) {
  int d = dist(a, b);
  if (d < dbest) {
    dbest = d;
    xbest = bx;
...
void improve_guess(cv::Mat *a, cv::Mat *b, int ax, int ay, int &xbest, int &ybest, int &dbest, int bx, int by, int patch_w) {
...
*/
void improve_guess(BITMAP *a, BITMAP *b, int ax, int ay, int &xbest, int &ybest, int &dbest, int bx, int by, int patch_w) {
  int d = dist(a, b, ax, ay, bx, by, patch_w, dbest);
  if (d < dbest) {
    dbest = d;
    xbest = bx;
...
int dist(BITMAP *a, BITMAP *b, int ax, int ay, int bx, int by, int patch_w, int cutoff) {
  int ans = 0;
...
int dist(cv::Mat *a, cv::Mat *b, int ax, int ay, int bx, int by, int patch_w, int cutoff) {
  int ans = 0;
  for (int dy = 0; dy < patch_w; dy++) {
    cv::Mat arow = a->rowRange( ay + dy , ax );
    cv::Mat brow = b->rowRange( by + dy , bx );
    for (int dx = 0; dx < patch_w; dx++) {
      int ac = arow.at<int>(dx);
      int bc = brow.at<int>(dx);
      int dr = (ac&255)-(bc&255);
      int dg = ((ac>>8)&255)-((bc>>8)&255);
      int db = (ac>>16)-(bc>>16);
      ans += dr*dr + dg*dg + db*db;
