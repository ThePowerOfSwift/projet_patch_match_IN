      ...
      for (int ax = xstart; ax != xend; ax += xchange) { 
        ...
        if ((unsigned) (ax - xchange) < (unsigned) aew) {
          ...
          if ((unsigned) xp < (unsigned) bew) {
            improve_guess(a, b, ax, ay, xbest, ybest, dbest, xp, yp, patch_w);
          }
        }
...
void patchmatch(cv::Mat *a, cv::Mat *b, cv::Mat *ann, cv::Mat *annd) {
  ...
  for (int ay = 0; ay < aeh; ay++) {
    for (int ax = 0; ax < aew; ax++) {
      ...
      annd->at<int>(ay,ax) = dist(a, b, ax, ay, bx, by, patch_w); 
}
  }
  for (int iter = 0; iter < pm_iters; iter++) {
    /* In each iteration, improve the NNF, by looping in scanline or reverse-scanline order. */
  ...
  save_bitmap(test1, "results/annd_matToBITMAP_Mode2.jpg");
  save_bitmap(test2, "results/ann_matToBITMAP_Mode2.jpg");
  delete test1;
  delete test2;
    ...
    for (int ay = ystart; ay != yend; ay += ychange) {
      for (int ax = xstart; ax != xend; ax += xchange) { 
        ...
        if ((unsigned) (ax - xchange) < (unsigned) aew) {
          ...
          int xp = INT_TO_X(vp) + xchange, yp = INT_TO_Y(vp);
          if ((unsigned) xp < (unsigned) bew)
            improve_guess(a, b, xbest, ybest, dbest, xp, yp);
        }
...
void patchmatch(cv::Mat *a, cv::Mat *b, cv::Mat *ann, cv::Mat *annd,cv::Mat *knn, cv::Mat *knnd) {
  ...
  for (int ay = 0; ay < aeh; ay++)
    for (int ax = 0; ax < aew; ax++) {
      (knn->at<cv::Vec<int, k> >(ay,ax))= cv::Vec<int, k>();
      (knnd->at<cv::Vec<int, k> >(ay,ax))= cv::Vec<int, k>();

      for (int i = 0; i < k; i++) {
          int bx = rand()%bew;
          int by = rand()%beh;

          (knn->at<cv::Vec<int, k> >(ay,ax))[i]= XY_TO_INT(bx, by);
          (knnd->at<cv::Vec<int, k> >(ay,ax))[i]= brent (a, b, a_brent, b_brent, eps_brent, t_brent, &x_brent, ax, ay, bx, by, patch_w);
      }      
    }
    ...
    for (int ay = ystart; ay != yend; ay += ychange) {
      for (int ax = xstart; ax != xend; ax += xchange) { 
        ...
        if ((unsigned) (ax - xchange) < (unsigned) aew) {
          ...
          if ((unsigned) xp < (unsigned) bew) {
            improve_guess(a, b, xbest, ybest, dbest, xp, yp);
            k_improve_guess(a,b,xbest,ybest,kdbest,xp,yp,guess_ok);
          }
        }
  ...
  cv::Mat * knnd1 = new cv::Mat_< cv::Vec<int, k> >(a->cols, a->rows);;

  for (int ay = 0; ay < aeh; ay++)
    for (int ax = 0; ax < aew; ax++) {
      (knn1->at<cv::Vec<int, k> >(ay,ax))= (knn->at<cv::Vec<int, k> >(ay,ax))[0];
      (knnd1->at<cv::Vec<int, k> >(ay,ax))= (knnd->at<cv::Vec<int, k> >(ay,ax))[0];
    }
